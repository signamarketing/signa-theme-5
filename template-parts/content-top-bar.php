



<?php

if( have_rows('top_bar', 'option') ): ?>
  <section class="" id="top-bar">
    <div class="container">
      <div class="row py-3">
        <?php while( have_rows('top_bar', 'option') ): the_row(); ?>
          <div class="col d-flex align-items-center">
            <p class="topbar-tagline text-white mb-md-0"><?php echo the_sub_field('tagline_text', 'option'); ?></p>
          </div>
        <?php endwhile; ?>
        <div class="col d-flex align-items-center justify-content-end">
        <?php while( have_rows('top_bar', 'option') ): the_row(); ?>
          <p class="top-bar-cta text-white mb-0"><?php echo the_sub_field('cta_text', 'option'); ?></p>
        <?php endwhile; ?>
          <a href="tel:<?php echo the_field('compnay_phone_number', 'option'); ?>" class="text-white font-weight-bold d-flex align-items-center">
          <img class="phone-icon" src="data:image/png;base64,"><?php echo the_field('compnay_phone_number', 'option'); ?></a>
        </div>
      </div>
    </div>
  </section>
  <?php endif; ?>
